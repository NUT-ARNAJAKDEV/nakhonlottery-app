...
import { FaCircleCheck, FaCircleXmark, FaClock, FaEye, FaEyeLowVision, FaPlus, FaPenToSquare, FaTrash, FaSistrix, FaListCheck, FaCartShopping, FaCartArrowDown, FaXmark } from 'react-icons/fa6';
...
function Order() {
    ...
    const handleDateChange = (date) => {
        setPaymentData({
            ...paymentData,
            paymentDate: formatToThaiDate(date),
        });
        setShowDatePicker(false);
    };

    // ฟังก์ชันเมื่อเลือกเวลา
    const handleTimeChange = (time) => {
        setPaymentData({
            ...paymentData,
            paymentTime: formatToTime(time),
        });
    ...
    const toggleOrderList = async () => {
        if (!currentUser) {
            alert('กรุณาล็อกอินเพื่อดูคำสั่งซื้อของคุณ');
            return;
        }
    ...
    const updateOrderStatus = async (orderId, status) => {
        try {
            ...
            alert(`อัปเดตสถานะคำสั่งซื้อเป็น "${status}" เรียบร้อยแล้ว`);
        } catch (error) {
                            ...
                            <div key={order.docId} className={styles.order_items}>
                                ...
                                <div className={styles.order_actions}>
                                    <button
                                        ...
                                        disabled={isDeleting || currentAmount <= 0}
                                        onClick={() => { addToCart(order); toggleCart(); }}>
                                        <FaCartArrowDown />
                                    </button>
                                </div>
                            </div>
                            ...
                            <>
                                ...
                                <div className={styles.cart_summary}>
                                                ...
                                                <div className={styles.form_row}>
                                                    <div className={styles.form_group}>
                                                        ...
                                                        <div className={styles.input_with_button}>
                                                            ...
                                                            {showDatePicker && (
                                                                <div className={styles.datepicker_popup}>
                                                                    <DatePicker
                                                                        selected={paymentData.paymentDate ?
                                                                            new Date(paymentData.paymentDate.replace(/(\d{2})\/(\d{2})\/(\d{2})/, (_, dd, mm, yy) => {
                                                                                const year = parseInt(yy) > 50 ? parseInt(yy) + 1900 : parseInt(yy) + 2000;
                                                                                return `${mm}/${dd}/${year - 543}`;
                                                                            })) : null}
                                                        ...
                                                        <div className={styles.input_with_button}>
                                                            ...
                                                            {showTimePicker && (
                                                                <div className={styles.timepicker_popup}>
                                                                    <DatePicker
                                                                        selected={paymentData.paymentTime ?
                                                                            new Date(`1970-01-01T${paymentData.paymentTime}`) : null}
                                    ...
                                    <button
                                        ...
                                        onClick={() => { handleCheckout(); setTimeout(() => { toggleCart(); }, 1000); setTimeout(() => { toggleOrderList(); },1500)}}>
                                        {isProcessing ? (
                                            <>
                ...
                <div className={styles.order_list_overlay}>
                    <div className={styles.order_list_modal}>
                        <div className={styles.order_list_header}>
                            <h2>คำสั่งซื้อทั้งหมด</h2> {/* เปลี่ยนข้อความเพื่อให้ชัดเจนว่าเป็นคำสั่งซื้อของผู้ใช้ */}
                            <button
                                onClick={toggleOrderList}
                                ...
                                {searchResult.found ? (
                                    <div className={styles.product_found}>
                                        ...
                                        <div className={styles.search_result_actions}>
                                            <button
                                                className={styles.delete_button}
                                                onClick={() => {addToCart(searchResult.product); handleCloseSearch(); toggleCart();}}
                                                disabled={isDeleting}
                                            >
                                                <FaCartArrowDown /> เพิ่มสินค้าลงรถเข็น
                                            </button>
                                        </div>
                                    </div>
                                ) : (
